...
main(int argc, const char* argv[]) {
    ...
    int total_len = 0;
    int ip_total_len = 0;
    char *interface,*srcIP, *srcMac, *desIP, *desMac;

    pcap_t* pcap = NULL;

    if (argc<2) {
//        fprintf(stderr,"usage: send_ip <interface>\n");
        fprintf(stderr,"Usage : traffic_generator <packet number> <interface> <TCP|UDP> <srcMac> <desMac> <srcIP> <desIP> <VLAN> <Pbit>\n");
        exit(1);
    }
/*	
	redisContext *redis = redisConnect("192.168.32.88",6379);
	redisReply *reply;
	if (redis !=NULL&& redis->err){
		printf("Error: %s\n",redis->errstr);
	}
	cJSON *json, *obj, *json1, *obj1;
	json = cJSON_Parse(system_setup);
	json = cJSON_GetObjectItem(json, "Redis");
	obj = cJSON_GetObjectItem(json, "setup");
//	printf("%s\n",obj->valuestring);
	reply = redisCommand(redis, "HGET %s neil",obj->valuestring);
	printf("%s\n",reply->str);
	json1 = cJSON_Parse(reply->str);
	obj1 = cJSON_GetObjectItem(json1,"interface");
	if (NULL==json1){
		printf("json null");
	}
	if (NULL==obj1){
...
//	printf("%s\n",obj1->valuestring);	
	interface = obj1->valuestring;
	obj1 = cJSON_GetObjectItem(json1,"srcIP");
	srcIP = obj1->valuestring;
	obj1 = cJSON_GetObjectItem(json1,"desIP");
	desIP = obj1->valuestring;
	obj1 = cJSON_GetObjectItem(json1,"srcMac");
	srcMac = obj1->valuestring;
	obj1 = cJSON_GetObjectItem(json1,"desMac");
	desMac = obj1->valuestring;
//	interface = "eth2";
//	printf("%s\n",interface);
...
*/
    ...
    set_eth(eth, srcMac,desMac, ETHERTYPE_IP);

    //check 8021Q
    if (argc>8){vlan_flag=1;}
    switch (vlan_flag){
        case 0:
            ip = frame + ETH_HDRLEN;
        ...
        case 1:
            ...
            set_vlan(vlan, atoi(argv[9]), 0, atoi(argv[8]));
    ...
    //source ip, dst ip
    set_ip(ip, srcIP, desIP, ip_type, ip_total_len);
